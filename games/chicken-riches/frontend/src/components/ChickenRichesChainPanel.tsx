...
const initialLeaderboardTop: LeaderboardEntry[] = [
  { name: "BARN", score: 1480 },
  { name: "YOLK", score: 1120 },
  { name: "HEN", score: 940 },
  { name: "COOP", score: 720 },
  { name: "CORN", score: 510 },
];

export type ChickenRichesPlayAccess = {
  canPlay: boolean;
  title: string;
  description: string;
};

type ChickenRichesChainPanelProps = {
  score: number;
  status: GameStatus;
  gameSessionId: number;
  autoSubmitOnGameOver?: boolean;
  onPlayAccessChange?: (state: ChickenRichesPlayAccess) => void;
...
export function ChickenRichesChainPanel({
  score,
  status,
  gameSessionId,
  autoSubmitOnGameOver = false,
  onPlayAccessChange,
  onSessionSubmitStateChange,
}: ChickenRichesChainPanelProps) {
  ...
  const autoSubmitAttemptedSessionIdRef = useRef<number | null>(null);
  const [playAccess, setPlayAccess] = useState<ChickenRichesPlayAccess>({
    canPlay: false,
  ...
  const finalRunScore = status === "gameOver" ? score : 0;
  ...
  const submitDisabledReason = useMemo(() => {
    ...
    if (status !== "gameOver") return "finish the current run first";
    if (isSubmittedForCurrentSession) return "score already submitted for this run";
    if (finalRunScore <= 0) return "score some points first";
    if (!isApiReady || !api) return "network is connecting";
    if (!programId) return "Chicken Riches program id is not configured";
  ...
  useEffect(() => {
    ...
    autoSubmitAttemptedSessionIdRef.current = null;
  ...
  const refreshVoucherState = useCallback(async () => {
    ...
    if (!connectedAccountAddress) {
      ...
      setPlayAccess({
        ...
        description: "Connect your Vara wallet in the panel to unlock the coop and start playing.",
      });
      return;
    }
    ...
    if (!programId) {
      setVoucherMessage("Chicken Riches program id is not configured.");
      setPlayAccess({
        canPlay: false,
        title: "Program unavailable",
        description: "Set VITE_PROGRAM_ID after deploying the Chicken Riches contract to unlock on-chain play.",
      });
      return;
    }
    ...
    setPlayAccess({
      ...
      description: "Checking your voucher and wallet balance before unlocking the coop.",
    });
  ...
  const submitScoreOnChain = useCallback(async () => {
    ...
    try {
      ...
      if (!programId) {
        setSubmitStatus("error");
        setSubmitMessage("Chicken Riches program id is not configured");
        return;
      }

      setSubmitMessage("Requesting gas voucher for this Chicken Riches score.");
      ...
      setSubmitMessage(
        isSignatureRejection(error)
          ? "The signature request was closed. Use Submit Score to try again."
  ...
  useEffect(() => {
    if (!autoSubmitOnGameOver) {
      return;
    }

    if (status !== "gameOver" || finalRunScore <= 0 || isSubmittedForCurrentSession || submitStatus === "pending") {
      return;
    }

    if (submitDisabledReason || autoSubmitAttemptedSessionIdRef.current === gameSessionId) {
      return;
    }

    autoSubmitAttemptedSessionIdRef.current = gameSessionId;
  ...
  const buttonLabel =
    submitStatus === "pending" ? "Submitting..." : isSubmittedForCurrentSession ? "Submitted For This Run" : "Submit Score";
  const isSubmitDisabled = Boolean(submitDisabledReason) || submitStatus === "pending";
  const resultHint =
    submitMessage ||
    (status === "gameOver"
      ? finalRunScore > 0
        ? "Run over. The wallet signature opens automatically to submit this score."
        : "This run ended without a score to submit."
      : "Submit unlocks only after the current run ends.");

  return (
    <>
      <section className="coop-panel coop-chain-panel" aria-label="Wallet and chain">
        <p className="coop-panel-kicker">Wallet & Submit</p>
        <h2>On-chain Score</h2>
        <div className="coop-wallet-widget" aria-label="Wallet connection">
          <Wallet theme="vara" displayBalance={false} />
        </div>

        <button className="coop-primary-button coop-chain-submit" type="button" onClick={submitScoreOnChain} disabled={isSubmitDisabled}>
          {buttonLabel}
        </button>

        {submitDisabledReason && submitStatus !== "pending" ? <p className="coop-chain-empty">{submitDisabledReason}</p> : null}
        <p className={`coop-chain-note coop-chain-note--${submitStatus}`}>{resultHint}</p>
      </section>

      <section className="coop-panel" aria-label="Gas voucher">
        <p className="coop-panel-kicker">Gas Voucher</p>
        <h2>Voucher Access</h2>
        <p className="coop-chain-note coop-chain-note--neutral">
          {voucherMessage || "Voucher will be checked automatically as soon as the wallet connects."}
        </p>
      </section>

      <section className="coop-panel" aria-label="On-chain leaderboard">
        <p className="coop-panel-kicker">Leaderboard</p>
        <h2>Top Farmers</h2>
        {chainStatusMessage ? <p className="coop-chain-empty">{chainStatusMessage}</p> : null}
        <div className="coop-chain-rows">
          {leaderboardTop.map((entry, index) => (
            <div className={`coop-chain-row${entry.name === CURRENT_PLAYER_NAME ? " is-current" : ""}`} key={`${entry.name}-${index}`}>
              <span>{index + 1}</span>
          ...
          {shouldShowCurrentPlayerRank && currentPlayerEntry && currentPlayerRank !== null ? (
            <div className="coop-chain-row is-current">

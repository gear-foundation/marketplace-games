...
export function describeVoucher(state: VoucherState, programId: `0x${string}`) {
  ...
  const hasProgramAccess = state.programs?.some((program) => program.toLowerCase() === programId.toLowerCase());
  const accessText = hasProgramAccess ? "Voucher ready" : "Voucher found";

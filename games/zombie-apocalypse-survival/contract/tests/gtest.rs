use ::zombie_apocalypse_survival_contract_client::{
    ZombieApocalypseSurvivalContractClient as _,
    ZombieApocalypseSurvivalContractClientCtors as _,
    game::Game as _,
};
...
async fn stores_only_best_score_and_updates_on_improvement() {
    ...
    let first = game.submit_score(2_400).await.unwrap();
    assert!(first.improved);
    assert_eq!(first.best_score, 2_400);

    let worse = game.submit_score(1_250).await.unwrap();
    assert!(!worse.improved);
    assert_eq!(worse.best_score, 2_400);
    assert_eq!(game.player_best_score(player).await.unwrap(), 2_400);

    let better = game.submit_score(3_900).await.unwrap();
    assert!(better.improved);
    assert_eq!(better.best_score, 3_900);
    assert_eq!(game.player_best_score(player).await.unwrap(), 3_900);
...
async fn leaderboard_is_descending_and_stable_by_actor_id_for_ties() {
    ...
    let second_program = sails_rs::client::Actor::<
        ::zombie_apocalypse_survival_contract_client::ZombieApocalypseSurvivalContractClientProgram,
        GtestEnv,
    ...
    third_game.submit_score(4_700).await.unwrap();
    second_game.submit_score(6_000).await.unwrap();
    first_game.submit_score(6_000).await.unwrap();

    let leaderboard = first_game.leaderboard(10).await.unwrap();
    assert_eq!(leaderboard.len(), 3);
    assert_eq!(leaderboard[0].player, first_actor);
    assert_eq!(leaderboard[0].score, 6_000);
    assert_eq!(leaderboard[1].player, second_actor);
    assert_eq!(leaderboard[1].score, 6_000);
    assert_eq!(leaderboard[2].player, third_actor);
    assert_eq!(leaderboard[2].score, 4_700);
...
async fn deploy_program(
    ...
    GtestEnv,
> {
    env.deploy::<::zombie_apocalypse_survival_contract_client::ZombieApocalypseSurvivalContractClientProgram>(
        program_code_id,
        b"salt".to_vec(),
    )
    .create()
...
fn create_env() -> (GtestEnv, CodeId) {
    ...
    let code_id = system.submit_code(::zombie_apocalypse_survival_contract::WASM_BINARY);
